...
public class RecordingProvider extends ContentProvider 
{
    ...
    public int delete(Uri uri, String where, String[] whereArgs) {
        ...
        case RECORDING_ID:
            ...
            count = recordingsDB.delete(RECORDINGS_TABLE, KEY_ID + "=" + segment + (!TextUtils.isEmpty(where) ? " AND (" + where + ')' : ""), whereArgs);
            break;
    ...
    public int update(Uri uri, ContentValues values, String where, String[] whereArgs) {
        int count;
        switch (uriMatcher.match(uri)) {
        case RECORDINGS: count = recordingsDB.update(RECORDINGS_TABLE, values, where, whereArgs);
            break;
            
        case RECORDING_ID: String segment = uri.getPathSegments().get(1);
            count = recordingsDB.update(RECORDINGS_TABLE, values, KEY_ID + "=" + segment + (!TextUtils.isEmpty(where) ? " AND (" + where + ')' : ""), whereArgs);
            break;
